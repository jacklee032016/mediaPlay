...
static int _decode_packet(int *got_frame, int cached)
{
	int ret = 0;
...
static int _open_codec_context(int *stream_idx, AVCodecContext **dec_ctx, AVFormatContext *fmt_ctx, enum AVMediaType type)
{
...
static int _get_format_from_sample_fmt(const char **fmt, enum AVSampleFormat sample_fmt)
{
	int i;
...
int main (int argc, char **argv)
{
	...
	if (_open_codec_context(&video_stream_idx, &video_dec_ctx, fmt_ctx, AVMEDIA_TYPE_VIDEO) >= 0)
	{
	...
	if (_open_codec_context(&audio_stream_idx, &audio_dec_ctx, fmt_ctx, AVMEDIA_TYPE_AUDIO) >= 0)
	{
	...
	while (av_read_frame(fmt_ctx, &pkt) >= 0)
	{
		...
		do
		{
			ret = _decode_packet(&got_frame, 0);
			if (ret < 0)
				break;
	...
	if (audio_stream)
	{
		...
		if ((ret = _get_format_from_sample_fmt(&fmt, sfmt)) < 0)
			goto end;

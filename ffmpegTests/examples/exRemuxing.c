...
static void _remuxing_log_packet(const AVFormatContext *fmt_ctx, const AVPacket *pkt, const char *tag)
{
...
int main(int argc, char **argv)
{
	...
	for (i = 0; i < ifmt_ctx->nb_streams; i++)
	{
...
#else
		/* output raw video format: only one stream */
		...
		if(in_codecpar->codec_type == AVMEDIA_TYPE_AUDIO)
		{
			printf("Audio framesize:%d; cide_tag:%d; extra_size:d; bit_rate:%d\n", in_codecpar->frame_size, in_codecpar->codec_tag, in_codecpar->extradata_size, in_codecpar->bit_rate);
			out_stream->time_base = (AVRational) {1, 25};
		}
		else if (in_codecpar->codec_type == AVMEDIA_TYPE_VIDEO)
		{
			printf("Video Stream: framesize:%d; cide_tag:%d; extra_size:d; bit_rate:%d\n", in_codecpar->frame_size, in_codecpar->codec_tag, in_codecpar->extradata_size, in_codecpar->bit_rate);
			out_stream->r_frame_rate = (AVRational ){ 25, 1};
			out_stream->avg_frame_rate = (AVRational){ 25, 1};
			out_stream->codec->time_base = (AVRational) {1, AV_TIME_BASE};// AV_TIME_BASE};
			out_stream->time_base = (AVRational) {1, AV_TIME_BASE};// AV_TIME_BASE};
		}
	...
	while (1)
	{
		...
		_remuxing_log_packet(ifmt_ctx, &pkt, "in");
...
#endif
		pkt.pos = -1;
		_remuxing_log_packet(ofmt_ctx, &pkt, "out");

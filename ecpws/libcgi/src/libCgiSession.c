...
*   One of the most diferencials of lib is its support to sessions, a mechanism that permits
...
* 	And yes, I ( me, it's author ) knows that there are problems yet, so your opinion is very important.
...
#ifdef	WIN32
char SESSION_SAVE_PATH[255] = "d:\\works\\temp\\";
#else
...
int sess_create_file()
{
	...
	if (!sess_file) {
		...
		libcgi_error(E_WARNING,"%s:%s", session_error_message[session_lasterror], sess_fname);
...
* After session_destroy() was called, is not more possible to use session functions before an another call to session_start()
...
/**
* Defines the name of the cookie that will use to store session's ID.
...
* If in the your CGI you don't make a call to cgi_session_save_path(), will
...
* // which will use to store session files.
* 
* cgi_session_save_path("session_files/");
*  
*  Note that using this form, will search for "session_files" directory using relative path 
...
* Resuming, the path is relative to where your application resides. <br><br>And remember, \b does \b not
...
int cgi_session_register_var(const char *name, const char *value)
{
	...
	if (!cgi_session_var_exists(name)) {
		...
		libcgiLog("session register variable %s=%s\n", data->name, data->value);

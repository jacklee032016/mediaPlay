...
CGI_TRACE()	;
		cntType = getenv("CONTENT_TYPE");
		if(cntType == NULL)
		{
CGI_TRACE()	;
			return NULL;
		}
		if( tmp_data =strstr(cntType, "multipart/form-data;"))
		{
CGI_TRACE()	;
			cntType = strstr(tmp_data, "boundary=");
			if(cntType)
			{
CGI_TRACE()	;
				return libcgiUpload(cntType+9, &formvars_start, &formvars_last);
			}
			else
			{
CGI_TRACE()	;
				libcgi_error(E_WARNING, "no 'boundary=' found in multipart form data\n");
				return NULL;
			}
		}
...
* <b>Note:</b><br>
* Does not implement RFC 2396 to make the lib simple and quick. You should be sure
...
/**
*  Configures all (most?) we need to  get cgi library working correctly. It MUST be called before any other cgi function.
**/
int cgi_init()
{
	init_hex_table();

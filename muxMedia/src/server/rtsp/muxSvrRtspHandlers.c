...
int muxSvrRtspEventErrorInfo(SERVER_CONNECT *svrConn, int statusCode)
{
	...
	MUX_SVR_ERROR( "RTSP Reply Error to client, error code : %d", statusCode);
...
{
	MuxStream *stream = muxSvrFoundStream(TRUE, path,  &svrConn->muxSvr->svrConfig );
	if(stream)
	{
...
int rtspEventDescribe(SERVER_CONNECT *svrConn, MUX_SVR_EVENT *event)
{
	...
	if(stream->acl != NULL && stream->acl->action != MUX_SVR_ACL_DISABLE)
	{
		...
		{
			MUX_SVR_ERROR("Remote IP Address is denied\n");
			return muxSvrRtspEventErrorInfo(svrConn, RTSP_STATUS_UNAUTHORIZED);
		}
		else
		{
			MUX_SVR_DEBUG( "Remote IP Address is allowed\n");
		}
	}
#endif
...
#define	CLOSE_DIRECTLY		1
	...
	{/* no other RTP connection for this RTSP connection */
		MUX_SVR_ERROR( "No more RTP connection exist, closing whole connection" );
		return MUX_SVR_STATE_CLOSING;
	}
...
//				if (st->feed->nbInStreams != 1)
				if (st->feed->mediaCapture->nbStreams != 1)
				{
...
//			for(streamIndex = 0; streamIndex < st->feed->nbInStreams; streamIndex++)
			for(streamIndex = 0; streamIndex < st->feed->mediaCapture->nbStreams; streamIndex++)
			{
	...
	if(stream==NULL)
	{
		MUX_SVR_ERROR( "Stream with name of '%s' is not found", rtspEvent->path );
		return muxSvrRtspEventErrorInfo(svrConn, RTSP_STATUS_SESSION);
	}
	
	if(streamIndex == -1)
	{/* no stream found */
		MUX_SVR_ERROR( "Stream index is not validate" );
	...
	else
	{
		...
		{/* if stream has already set up, so no 'SETUP' cmd is needed */
			MUX_SVR_ERROR( "RTP AvFormat of Stream with name of '%s' index is '%d' has exist", rtspEvent->path, streamIndex );
			return muxSvrRtspEventErrorInfo(svrConn, RTSP_STATUS_STATE);
		}
	}

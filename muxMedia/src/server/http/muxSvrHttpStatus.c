...
static int  _streamStatus(int index, void *ele, void *priv)
{
	...
	url = muxSvrStreamUrl(stream);
	format = muxSvrStreamFormat(stream);
	muxSvrConnectPrintf(svrConn, "<TR><TD><A HREF=\"%s\">%s</A> <TD align=center> %s ", url, url, format );
	cmn_free(url);
	cmn_free(format);
...
static int _printFeedInfo(int index, void *ele, void *data)
{
	int i;
	MuxFeed  *feed = (MuxFeed *)ele;
	SERVER_CONNECT *svrConn = (SERVER_CONNECT *)data;
	char parameters[128];
VTRACE();
...
VTRACE();
		...
		if(!desc)
		{
			CMN_ABORT("No media descriptor is found in stream '%d'", i);
		}
#endif
VTRACE();
...
//				snprintf(parameters, sizeof(parameters), "%d channel(s), %d Hz", st->codecpar->channels, st->codecpar->sample_rate);
			...
			default:
				CMN_ABORT("Invalidate media type: '%s'", av_get_media_type_string(st->codecpar->codec_type) );
		}
...
VTRACE();
	...
	{
		CMN_ABORT( "%s has no stream ", svrConn->name );
	}
#endif
...
VTRACE();
	muxSvrConnectPrintf(httpConn, "<TR><TD><B>%d</B><TD>%s<TD>%s<TD>%s<TD>%s<td align=right>", 
			index, url, ip, muxSvrConnTypeName(svrConn->type), muxSvrConnStatusName(svrConn->state) ); 
...
#if 1
	/* list all feeds, no matter what type it is */
VTRACE();
	if(cmn_list_iterate(&muxSvr->svrConfig.feeds, TRUE, _printFeedInfo, svrConn) != EXIT_SUCCESS)
	{
	}
#else
	while(feed)
...
VTRACE();
	...
	while(feed)
	{
		/* feeds in this list are File Feed serviced by FileFeed Thread */

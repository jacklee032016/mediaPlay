...
//	svrConn->outFmtCtx->pb->is_streamed = 1;
	...
	if(IS_STREAM_FORMAT(ctx->oformat, "flv") )
	{
		MUX_SVR_DEBUG("'%s' : add BSF of aac_adtstoasc for AAC in FLV output format", svrConn->name );
		ret = av_dict_set(&headerOptions, "fflags", "-autobsf", 0);
		ret |= av_dict_set(&headerOptions, "bitstream_filters", "aac_adtstoasc", 0);
		if (ret < 0)
		{
			MUX_SVR_ERROR( "Set dictionary failed: %s\n", av_err2str(ret));
		}
	}
#endif
	...
	{
		MUX_SVR_DEBUG("'%s' : add BSF of h264_mp4toannexb in AVI output format", svrConn->name );
		ret = av_dict_set(&headerOptions, "fflags", "-autobsf", 0);
...
VTRACE();
//	ret = avformat_write_header(svrConn->outFmtCtx, NULL);//&headerOptions);
	ret = avformat_write_header(svrConn->outFmtCtx, &headerOptions);
	if (ret < 0)
	{
		MUX_SVR_ERROR( "Error write header in HTTP '%s' connections: %s", svrConn->name, av_err2str(ret));
		return -EXIT_FAILURE;
	}
...
int httpEventLiveStream(SERVER_CONNECT *svrConn, MUX_SVR_EVENT *event)
{
	...
	MUX_SVR_DEBUG( "Connection enter into state of 'HTTPSTATE_SEND_HEADER'");
...
static void _httpReplyError(SERVER_CONNECT *svrConn, char *errMsg, int errNo )
{
	...
	MUX_SVR_ERROR( "HTTP Request error: %s", errMsg);
}
...
//	stream = muxSvrFoundStream(TRUE, svrConn->method+1, &svrConn->muxSvr->svrConfig);
	stream = muxSvrFoundStream(TRUE, svrConn->filename, &svrConn->muxSvr->svrConfig);

...
//		nbStreams = svrConn->stream->feed->nbInStreams;
		nbStreams = svrConn->stream->feed->mediaCapture->nbStreams;
	}
...
//					svrConn->fd = -1;
				...
				{
					MUX_SVR_ERROR( "%s listening connection is closed", svrConn->name );
					exit(1);
				}
#endif				
			}
...
static int _ctrlFsmOnEvent(CmnThread *th, void *_event)
{
	int		j;
...
//	MUX_SVR_DEBUG("'%s' return new state of %d from state of %d", svrConn->name, newState, svrConn->state );
	if(newState != MUX_SVR_STATE_CONTINUE && newState != svrConn->state )
	{
		...
		MUX_SVR_DEBUG("'%s' enter into new state of %s from state of %s", svrConn->name, muxSvrConnStatusName(newState),  muxSvrConnStatusName(svrConn->state) );
...
static int	_muxSvrHandler(CmnThread *th, void *_event)
{
	MUX_SVR *muxSvr =(MUX_SVR *)th->data;
	MUX_SVR_EVENT *svrEvent = (MUX_SVR_EVENT *)_event;

	int ret = EXIT_SUCCESS;

	if(CHECK_FSM_EVENT( svrEvent->event) )
	{/* FSM events come from SERVER itself */

		CLAER_FSM_EVENT_FLAGS(svrEvent->event);

		ret = _ctrlFsmOnEvent(th, _event);
		if(ret < 0)
		{
			MUX_SVR_ERROR( "FSM returned error %d, program exit", ret );
			return ret;
		}
		/* free FSM event by server itself */
		av_free(_event);
	}
	else
	{/* jSonEvent, API event from controller. jsonEvent is freed after reply it automatically */
		...
		MUX_SVR_DEBUG("SERVER handle JSON/API event");
		ret = cmnMuxJSonPluginHandle(muxSvr, muxSvrJsonHandlers, jsonEvent);
	}
#if 0	
...
static int _muxSvrCtrlMain(CmnThread *th)
{
	MUX_SVR *muxSvr =(MUX_SVR *)th->data;
	struct timeval		tv;
	int				ret;
	...
	if( 0)//SYSTEM_IS_EXITING() )
	{
		MUX_SVR_ERROR( "Server Task recv EXIT signal\n" );
		
#if WITH_SERVER_WATCHDOG
		/* when server normal exit, watchdog exit normally */
		if(muxSvr->wdStart)
		{
			MUX_SVR_ERROR( "WatchDog recv EXIT signal\n" );
...
//		tv.tv_usec = 100;
		...
		{
			MUX_SVR_ERROR( "select() returned error %d: %s", ret , strerror(errno));
	...
	muxSvr->currentTime = cmnGetTimeMs();
...
CmnThread  threadServer =
{
	name		:	"Controller",
	init			:	muxSvrCtrlInit,
	mainLoop		:	_muxSvrCtrlMain,
	eventHandler	:	NULL,//_muxSvrHandler,
	destory		:	NULL,
	data			:	NULL,
};

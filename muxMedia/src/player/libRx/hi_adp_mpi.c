...
HI_S32 HIADP_Disp_StrToFmt(HI_CHAR *pszFmt)
{
    ...
    else if (0 == strcasecmp(pszFmt, "4096P_24"))
	{
		fmtReturn = HI_UNF_ENC_FMT_4096X2160_24;
	}
    else if (0 == strcasecmp(pszFmt, "4096P_25"))
	{
		fmtReturn = HI_UNF_ENC_FMT_4096X2160_25;
	}
    else if (0 == strcasecmp(pszFmt, "4096P_30"))
	{
		fmtReturn = HI_UNF_ENC_FMT_4096X2160_30;
	}
    else if (0 == strcasecmp(pszFmt, "4096P_50"))
	{
		fmtReturn = HI_UNF_ENC_FMT_4096X2160_50;
	}
    else if (0 == strcasecmp(pszFmt, "4096P_60"))
	{
		fmtReturn = HI_UNF_ENC_FMT_4096X2160_60;
	}
    else
    {
        fmtReturn = HI_UNF_ENC_FMT_1080P_30;
        sample_common_printf("\n!!! Can NOT match format, set format to is '1080P_30'/%d.\n\n", HI_UNF_ENC_FMT_1080P_30);
    }
...
HI_S32 HIADP_Disp_Init(HI_UNF_ENC_FMT_E enFormat)
{
	...
	HI_U32						virWidth = 1920;
	HI_U32						virHeight = 1080;
	...
	/* DBG: Currently restrict max size to 1920x1080, since output error when setting input format to HI_UNF_ENC_FMT_3840X2160_60*/
	/* might be a buffer or image's line spacing problem*/
#if 0
	switch (enFormat)
	{
		case HI_UNF_ENC_FMT_4096X2160_60 :
		...
		case HI_UNF_ENC_FMT_480P_60 :	//the resolution is TBD
			virWidth = 640;
			virHeight = 480;
			break;

		case HI_UNF_ENC_FMT_NTSC :		//the resolution is TBD
			virWidth = 720;
			virHeight = 480;
			break;

		case HI_UNF_ENC_FMT_576P_50 :	//the resolution is TBD
		case HI_UNF_ENC_FMT_PAL :		//the resolution is TBD
			virWidth = 720;
			virHeight = 576;
			break;
	...
	Ret = HI_UNF_DISP_SetVirtualScreen(HI_UNF_DISPLAY1, virWidth, virHeight/*1280, 720*/);
	if (Ret != HI_SUCCESS)
	{
		MUX_PLAY_ERROR("HI_UNF_DISP_SetVirtualScreen %dx%d failed 0x%x\n", virWidth, virHeight, Ret);
		HI_UNF_DISP_Detach(HI_UNF_DISPLAY0, HI_UNF_DISPLAY1);
		HI_UNF_DISP_DeInit();
		exit(1);
		return Ret;
	...
	{
		MUX_PLAY_DEBUG("VirtualScreen success %d x %d\n", virWidth, virHeight);
	}

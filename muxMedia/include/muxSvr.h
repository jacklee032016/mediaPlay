...
/* file feed thread send data to capture, not send to stream directly */
#define	MUX_SVR_OPT_FILE_FEED_2_CAPTURE			1
...
//#if ARCH_ARM
#define	MUX_SERVER_CONFIG_FILE		CONFIG_FILE_HOME_PROJECT"muxServer.conf"
...
typedef struct _MuxFeed
{
	...
	/*when it is file feed, this is the name of capture which is registered in muxMain by this feed;
	* when it is capture feed, this is the name of capture this feed is looking for.
	*/
	...
	int						outSyncThreshold;	/* threshold of out of sync, in count of frame number */
	int						delayCount;		/* delay time in count of frame number */
	int						durationSecond;		/* second of this media */
	...
	/* following fields are for file feed primitive streams in media file  */
...
typedef	struct _MUX_SVR
{
	...
	MuxFeed					*feeds;		/* feeds which are added here is only for file feed service, feed thread uses it */
	int						feedThreadId;	/* <=0, thread not created; > 0, thread ID */
...
#define	SVR_REPORT_FSM_EVENT(event)	\
	cmnThreadAddEvent( &threadServer,  event)
...
#define  MUX_SVR_INFO(...)		{CMN_MSG_LOG(CMN_LOG_INFO, __VA_ARGS__);}

#define  MUX_SVR_WARN(...)		{CMN_MSG_LOG(CMN_LOG_WARNING, __VA_ARGS__);}

#define  MUX_SVR_ERROR(...)		{CMN_MSG_LOG(CMN_LOG_ERR, __VA_ARGS__);}
...
extern	CmnThread  	threadFileFeed;
extern	CmnThread  	threadServer;
...
extern	PluginJSonHandler muxSvrJsonHandlers[];


char *muxSvrConnStatusName(int	status);
char *muxSvrConnTypeName(conn_type_t type);

char *muxSvrStreamUrl(MuxStream *stream);
char* muxSvrStreamFormat(MuxStream *stream);

...
HI_VOID SocketThread(HI_VOID *args)
{
    ...
    {
        Ret = HI_UNF_DMX_GetTSBuffer(g_TsBuf, 188*500, &StreamBuf, 0);
        if (Ret != HI_SUCCESS)
		{
            GetBufCount++;
            if(GetBufCount >= 10)
...
HI_S32 main(HI_S32 argc,HI_CHAR *argv[])
{
    ...
    {
        stWinRect.s32Width = 1920 / (AVPLAY_NUM / 2);
        stWinRect.s32Height = 1080 / 2;
    ...
    while(1)
    {
        ...
        for ( i = 0 ; i < AVPLAY_NUM; i++ )
        {
            Ret = HIADP_AVPlay_PlayProg(hAvplay[i],g_pProgTbl,ProgNum,bAudPlay[i]);
            if (Ret != HI_SUCCESS)
            {

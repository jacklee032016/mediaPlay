...
static HI_BOOL s_NeedWait = HI_TRUE; //HI_FALSE;
...
{
	return Test_DrawTextTime(FONT_GB2312_DOTFILE, pTimeCosted);
}

HI_S32 Test_DrawTextTime_Ttf(HI_S32 *pTimeCosted)
{
	return Test_DrawTextTime(FONT_GB2312_TTFFILE, pTimeCosted);    
}
...
{
	return Test_DoDecImageTime(RES_HOME_DIR"/png/performance.png", pTimeCosted);
}

HI_S32 Test_DecImageTime_JPG(HI_S32 *pTimeCosted)
{
	return Test_DoDecImageTime(RES_HOME_DIR"/jpg/performance.jpg", pTimeCosted);
}

HI_S32 Test_DecImageTime_GIF(HI_S32 *pTimeCosted)
{
	return Test_DoDecImageTime(RES_HOME_DIR"/gif/performance.gif", pTimeCosted);
}

HI_S32 Test_DecImageTime_BMP(HI_S32 *pTimeCosted)
{
	return Test_DoDecImageTime(RES_HOME_DIR"/bmp/performance.bmp", pTimeCosted);    
}
...
HI_S32 Test_DisplayResult(HI_S32 *pResultTime, HI_S32 num)
{
	...
	/** create the object of text display */   
	ret = HI_GO_CreateText(FONT_GB2312_DOTFILE, NULL, &hText);
	if (HI_SUCCESS != ret)
	{
		Printf("failed to create the font: line%d,%s!\n", __LINE__,FONT_GB2312_DOTFILE);
		goto RET;
	}

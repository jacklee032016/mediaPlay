...
//				av_dict_set(&headerOptions, "hls_flags", "single_file", 0);//AV_OPT_SEARCH_CHILDREN);
	...
	if (!ctx)
	{
		MUX_SVR_LOG(ERROR_TEXT_BEGIN"'%s' Error: %s in alloc avformat context"ERROR_TEXT_END, svrConn->name, av_err2str(ret));
		return -EXIT_FAILURE;
	}
...
VTRACE();
	...
	if(ret < 0)
	{
		MUX_SVR_LOG(ERROR_TEXT_BEGIN"open %s IOContext failed: %s"ERROR_TEXT_END, svrConn->name, av_err2str(ret));
		goto fail;
	}
...
VTRACE();
	...
	{
fail:
		MUX_SVR_LOG(ERROR_TEXT_BEGIN"Write Header for %s failed: %s"ERROR_TEXT_END, svrConn->name, av_err2str(ret));
		av_free(ctx);
		return -EXIT_FAILURE;
	}
...
VTRACE();
	...
	if(svrConn->urlContext && len> 0)
	{
		...
		if(i != len)
		{
			MUX_SVR_LOG(ERROR_TEXT_BEGIN"Only %d bytes written in %d bytes"ERROR_TEXT_END, i, len );
		}
	}
...
static int _serviceConnStart(SERVER_CONNECT *svrConn, MUX_SVR_EVENT *event)
{
	...
	if(svrConn->type == CONN_TYPE_RTMP)
	{
		...
		{
			MUX_SVR_LOG(ERROR_TEXT_BEGIN"Error: RTMP stream must be flv format"ERROR_TEXT_END);
			exit(1);
			return EXIT_SUCCESS;
		}
	...
	else if(svrConn->type == CONN_TYPE_MULTICAST)
	{
		...
		{
			MUX_SVR_LOG(ERROR_TEXT_BEGIN"Error: Multicast stream must be mpegts format"ERROR_TEXT_END);
			exit(1);
			return EXIT_SUCCESS;
		}
	...
	else if(svrConn->type == CONN_TYPE_HLS)
	{
		...
		{
			MUX_SVR_LOG(ERROR_TEXT_BEGIN"Error: HLS stream must be hls format"ERROR_TEXT_END);
			exit(1);
			return EXIT_SUCCESS;
		}
	}
	...
	{
		MUX_SVR_LOG(ERROR_TEXT_BEGIN"Error: stream '%s' is invalidate format "ERROR_TEXT_END, stream->filename);
		exit(1);
	}
	...
	if(!IS_STRING_NULL(protocol))
	{
		...
		if (ret < 0)
		{
			MUX_SVR_LOG(ERROR_TEXT_BEGIN"RTMP URL initializing failed : %s"ERROR_TEXT_END, av_err2str(ret));
			exit(1);
			return EXIT_FAILURE;
		}
...
static int _serviceConnStop(SERVER_CONNECT *svrConn, MUX_SVR_EVENT *event)
{
	...
	{
		MUX_SVR_LOG(ERROR_TEXT_BEGIN"open dyn buffer failed for service connect '%s': %s"ERROR_TEXT_END, svrConn->name, av_err2str(ret));
		/* XXX: potential leak */
//		return -EXIT_FAILURE;
	}
	...
	if(svrConn->urlContext)
	{
		...
		if(ret != len)
		{
			MUX_SVR_LOG(ERROR_TEXT_BEGIN"Only write %d bytes of %d bytes when write trailer"ERROR_TEXT_END, ret, len);
		}
...
static int _svrInitGenericConn(int index, void *ele, void *priv)
{
	...
	if(stream == NULL)
	{
		MUX_SVR_LOG( "Stream of '%s' is found, please check your configuration"ERROR_TEXT_END, RECORD_STREAM_NAME);
		exit(1);
	}
	...
	{
		MUX_SVR_LOG("Invalidate Service Connection"ERROR_TEXT_END);
		av_free(svrConn);
		return EXIT_SUCCESS;
	}

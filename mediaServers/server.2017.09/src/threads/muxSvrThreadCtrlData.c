...
//			VTRACE();
		...
		/* for UDP multicast, RTSP connection */
			...
			{/* fail safe - should never happen */
				MUX_SVR_LOG(ERROR_TEXT_BEGIN"'%s' WARNS: Length of media data is too small : %d"ERROR_TEXT_END, svrConn->name, left);
fail1:
			...
			if (len > left )
			{
				MUX_SVR_LOG(ERROR_TEXT_BEGIN"'%s' WARNS: Length of this sub-packet data %d is larger than left %d"ERROR_TEXT_END, svrConn->name, len, left);
				goto fail1;
			}
...
//				svrConn->dataBuffer.bufferPtr;
	...
	//				dataConn->state = RTP_STATE_WAIT_SHORT;
		...
		if( len < 0)
		{
			...
			MUX_SVR_LOG(ERROR_TEXT_BEGIN"'%s' : error on stream %d of  is %s"ERROR_TEXT_END, svrConn->name, svrConn->currentStreamIndex, av_err2str(len));
...
static int _serviceConnectEncode(SERVER_CONNECT *svrConn, AVPacket *pkt)
{
	...
	if( IS_RTSP_CONNECT(svrConn) )
	{
		...
		{
			MUX_SVR_LOG(ERROR_TEXT_BEGIN"'%s' : No.%s Data Conn is null"ERROR_TEXT_END, svrConn->name );
			exit(1);
		}
	...
	{
		svrConn->firstPts = av_rescale_q(pkt->dts, ist->time_base, AV_TIME_BASE_Q);
	...
	{
		svrConn->currentFramePts = av_rescale_q(pkt->dts, ist->time_base, AV_TIME_BASE_Q);
		svrConn->currentFramePts -= svrConn->firstPts;
	}
	svrConn->currentFrameDuration = av_rescale_q(pkt->duration, ist->time_base, AV_TIME_BASE_Q);
#endif
...
//		VTRACE();
	...
	if (ret < 0)
	{
		MUX_SVR_LOG(ERROR_TEXT_BEGIN"'%s' : Error in open_dyn_buf : %s\n"ERROR_TEXT_END, svrConn->name, av_err2str(ret));
	}
	...
	if (ret < 0)
	{/* XXX: potential leak */
		MUX_SVR_LOG(ERROR_TEXT_BEGIN"'%s' : Error writing frame to output : %s\n"ERROR_TEXT_END, svrConn->name, av_err2str(ret));
		return ret;
	}
	...
	{
		MUX_SVR_LOG(ERROR_TEXT_BEGIN"'%s' : buffer is %d, smaller than packet length %d\n"ERROR_TEXT_END, svrConn->name, len, pkt->size );
	}
...
int muxSvrConnSendPacket(SERVER_CONNECT *svrConn, MUX_SVR_EVENT *_event)
{
	...
	if( left > 0)
	{
		...
		if(len < 0)
		{
			MUX_SVR_LOG("Write media data for '%s' error: %s: left: %d", svrConn->name, strerror(errno), left);
			HTTP_EMIT_EVENT(svrConn, MUX_SVR_EVENT_ERROR, SRV_HTTP_ERRCODE_CLOSED_BY_PEER);
		}
		else
		{
		...
		if(len < 0)
		{
			MUX_SVR_LOG(ERROR_TEXT_BEGIN"'%s' : Error in send out left data buffer error on "ERROR_TEXT_END, svrConn->name);
...
//VTRACE();
#if MUX_SVR_DEBUG_DATA_FLOW
	MUX_SVR_DEBUG("\n'%s' : send out a new packet: %d on stream %d", svrConn->name, pkt->size, pkt->stream_index);
#endif
	len = _serviceConnectEncode(svrConn, pkt);
	av_packet_free(&pkt);
	if(len < 0)
	{
//		MUX_SVR_LOG(ERROR_TEXT_BEGIN"'%s' : Encode AVPacket error"ERROR_TEXT_END, svrConn->name);
//		HTTP_EMIT_EVENT(svrConn, MUX_SVR_EVENT_CLOSE, 0);
		return MUX_SVR_STATE_CONTINUE;
	}
...
//VTRACE();
	...
	if(len < 0)
	{
		MUX_SVR_LOG(ERROR_TEXT_BEGIN"'%s' : send out data buffer error"ERROR_TEXT_END, svrConn->name);
		HTTP_EMIT_EVENT(svrConn, MUX_SVR_EVENT_CLOSE, 0);
	}

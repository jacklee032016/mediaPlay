...
int _mntrConnPollIn(SERVER_CONNECT *listenConn, MUX_SVR_EVENT *event)
{
	...
	{
		MUX_SVR_LOG( "Connection upper Limit '%d' has reached", listenConn->muxSvr->svrConfig.maxConnections);
		close(fd);
		return -EXIT_FAILURE;
	}
...
static int _mntrConnStart(SERVER_CONNECT *svrConn, MUX_SVR_EVENT *event)
{
	...
	if (serverFd < 0)
	{
		MUX_SVR_LOG( "Build Socket failed : %s", strerror(errno) );
	...
	{
		MUX_SVR_LOG( "Socket Bind failed on bind(port %d): %s", address->port, strerror(errno) );
		close(serverFd);
		return HTTP_STATE_ERROR;
	}
	...
	{
		MUX_SVR_LOG("Socket Listen failed : %s", strerror(errno) );
		close(serverFd);
		return HTTP_STATE_ERROR;
	}
	...
	MUX_SVR_DEBUG( "Monitoring IP : %s:%d on socket %d", address->address, address->port, svrConn->fd );

...
static void range_add(WEB_CLIENT_CONN * conn, unsigned long start, unsigned long stop)
{
    ...
    RANGE_T *r = range_pool_pop();
        ...
        WEB_MSG_DEBUG("range.c, range_add: got: %lu-%lu\n", start, stop);
...
int ranges_fixup(WEB_CLIENT_CONN * conn)
{
	...
	*/
	...
	while(r)
	{
		...
		*/
		WEB_MSG_DEBUG( "range.c: ranges_fixup: %lu-%lu\n", r->start, r->stop);
		...
		*/
		if ((long) r->start < 0 || r->start > r->stop)
		{
			...
			r = r->next;
			range_pool_push(temp);
			WEB_MSG_DEBUG( "start or end of range is invalid. skipping.\n");
			continue;
		}
		...
		WEB_MSG_DEBUG( "ending with start: %lu\tstop: %lu\n", r->start, r->stop);
	...
	WEB_MSG_DEBUG( "ranges_fixup returning 1\n");
	return 1;
}

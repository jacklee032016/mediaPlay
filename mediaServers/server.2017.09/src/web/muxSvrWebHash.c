...
static unsigned four_char_hash(const char *buf)
{
	...
	WEB_MSG_DEBUG("four_char_hash(%s) = %u\n", buf, hash);
	return hash;
}
...
//	WEB_MSG_DEBUG( "Adding key \"%s\" for value \"%s\" (hash=%d)\n", key, value, hash);
...
void hash_show_stats(void)
{
	...
	{ /* these limits OK? */
		if (mime_hashtable[i])
		{
			...
			WEB_MSG_DEBUG("mime_hashtable[%d] has %d entries\n", i, count);

			total += count;
		}
	}

	WEB_MSG_DEBUG("mime_hashtable has %d total entries\n", total);
	...
	{ /* these limits OK? */
		if (passwd_hashtable[i])
		{
			...
			WEB_MSG_DEBUG( "passwd_hashtable[%d] has %d entries\n", i, count);

...
trace();
		...
		*/
		...
		if (dest + howmuch > conn->buffer + BUFFER_SIZE)
		{
			/* big problem */
		...
		WEB_MSG_DEBUG("CONN%d: copied %d bytes into offset of %d\n", conn->index, howmuch, dest - conn->buffer);
...
WEB_CLIENT_STATUS ecpwsCgiHandlerReadPipe(WEB_CLIENT_CONN *conn)
{
	...
	WEB_MSG_DEBUG("buf:%p;header_end:%p(offset %d bytes), HeaderLine:%p(offset %d bytes) \n",conn->buffer, 
			conn->header_end, conn->header_end-conn->buffer, conn->header_line, conn->header_line-conn->buffer);
	bytes_to_read = BUFFER_SIZE - (conn->header_end - conn->buffer - 1);
...
//		WEB_MSG_DEBUG("CONN %d:Pipe - read %d bytes: \"%s\"\n",conn->index, bytes_read, conn->header_end);
	}
	else
	{
		WEB_MSG_DEBUG("CONN %d:Pipe -read %d bytes\n",conn->index, bytes_read);
	}
	WEB_MSG_DEBUG( "status:%d, cgi_status:%d\n", conn->status, conn->cgi_status);
	...
	if (bytes_read == -1)
	{
		...
		{
			ECPWS_LOG_REQ_ERR(conn, "pipe read");
			return WEB_CLIENT_S_ERROR;
		}
	}
...
//		WEB_MSG_DEBUG("buf:%p;header_end:%p(offset %d bytes), HeaderLine:%p(offset %d bytes) (%s)\n",conn->buffer, 
...
//	conn->header_line = conn->buffer+BUFFER_SIZE/2;

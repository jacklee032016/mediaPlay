	...
	switch (child_pid)
	{
		...
		default:
			...
			if (conn->cgi_type == CGI_T_CGI)
			{
				...
				conn->header_line = conn->header_end = (conn->buffer + BUFFER_SIZE / 2);
			}
			else
			{
...
trace();
	if (conn->cgi_type == CGI_T_CGI || conn->cgi_type == CGI_T_NPH)
	{
		...
		WEB_MSG_DEBUG("finding '%c' in conn->pathname: \"%s\"\n", DIR_SPLITTER_CHAR,conn->pathname);
#if 0
		char *c;
		unsigned int l;
		char *newpath, *oldpath;
...
trace();
		*c = '\0';
		...
		WEB_MSG_DEBUG("New working directory:'%s'\r\n", conn->pathname);
...
trace();
	...
	*/
	if (cgiLogFile)
	{
		WEB_MSG_DEBUG("set log file for CGI program\n");
...
//			execl(GUNZIP, GUNZIP, "--stdout", "--decompress",	conn->pathname, (void *) NULL);
	...
	WEB_MSG_DEBUG("CGI '%s' running OK\n", cmdLine);
	if (usePipes)
	{
...
trace();
...
#ifdef FASCIST_LOGGING
	{
		...
		for (i = 0; i < conn->cgi_env_index; ++i)
			WEB_MSG_DEBUG(" - environment variable for cgi: \"%s\"\n", conn->cgi_env[i]);
	}
#endif
...
trace();
	...
	if (conn->cgi_type == CGI_T_CGI ||(!conn->cgi_type &&(conn->pathname[strlen(conn->pathname) - 1] == '/')))
	{
		...
		if (set_nonblock_fd(pipeFds[0]) == -1)
		{
			ECPWS_LOG_REQ_NOERR(conn, "cgi-fcntl");
			CLOSE(pipeFds[0]);
			CLOSE(pipeFds[1]);

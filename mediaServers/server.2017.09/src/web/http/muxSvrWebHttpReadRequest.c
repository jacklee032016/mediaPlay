...
//	WEB_MSG_DEBUG("lookup in '%s', size of %d bytes\n", header, size)
#if 1
...
#endif	
//	WEB_MSG_DEBUG("count %d 0x%x\n", count, check[0] );
	if(check[0]=='\r' )
	{
//		WEB_MSG_DEBUG("find First slpit 0x%x 0x%x\n", check[1], check[2]);
...
//	WEB_CONFIG_T *cfg = conn->webSvr->cfg;
	...
	{
		buffer[bytes] = '\0';
//		WEB_MSG_DEBUG( "- Parsing headers (\"%s\")\n", check);
	}
			...
			*check = '\0';
			...
			if (conn->header_end - conn->header_line >= MAX_HEADER_LENGTH)
			{
				ECPWS_LOG_REQ_NOERR_SEND_BAD_REQ(conn, 
					"Header too long at %d bytes: \"%s\"\n", conn->header_end - conn->header_line, conn->header_line);
				return WEB_CLIENT_S_ERROR;
			}


//			WEB_MSG_DEBUG("parsing header '%s'\n", conn->header_line);
			...
			{
//				WEB_MSG_DEBUG("find End of HTTP REQ\n");

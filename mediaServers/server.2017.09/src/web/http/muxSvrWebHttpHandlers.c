...
static int _system_authen(char *user, char *pass)
{
	...
	WEB_MSG_DEBUG("authening user %s in /etc/passwd %s\n" , user, __FUNCTION__ );
...
/*	pw = my_getpwnam ( user );*/
	...
	WEB_MSG_DEBUG("password for %s user is %s, len is %d; pass user input is %s,len is %d\n", user , pw->pw_passwd, strlen(pw->pw_passwd), pass, strlen(pass) );
	...
	if (( strcmp ( pw->pw_passwd, "x" ) == 0 ) || ( strcmp ( pw->pw_passwd, "*" ) == 0 )) 
	{
		...
		WEB_MSG_DEBUG( "password of %s user is used\n" , user);
		...
		correct = sp->sp_pwdp;
		WEB_MSG_DEBUG( "correct='%s'  passwd='%s'\n", correct, pass); 

		encrypted = crypt ( pass,  correct );

		WEB_MSG_DEBUG("encrypted='%s'\n", encrypted); 
...
WEB_CLIENT_STATUS ecpwsHttpHandlerReadPost(WEB_CLIENT_CONN * conn)
{
	...
	bytes_free = BUFFER_SIZE - (conn->header_end - conn->header_line);
	...
	*/
	...
	WEB_MSG_DEBUG("- read %d bytes.\n\"", bytes_read);
	ecpwsOutputMsg(conn->header_end);
	ecpwsOutputMsg("\"\n");
...
//	WEB_MSG_DEBUG("Write: %d bytes : \"%s\"\n", bytes_written, conn->data_mem + conn->ranges->start);
	WEB_MSG_DEBUG("Write: %d bytes\n", bytes_written );

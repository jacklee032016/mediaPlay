...
//		conn->pathname[strlen(conn->pathname)-1] = '\0';
...
#endif
		WEB_MSG_DEBUG("%s is not a regular file, is %s a directoty\n", conn->pathname, (isDir == -1)?"not":"");
		if(isDir == -1)
		{
...
#if 0
	WEB_MSG_DEBUG( "if-range: %s\time_cval: %d\tmtime: %d\n",
	...
	*/
	...
	if (conn->data_mem)
	{
		/* things can really go tilt if conn->buffer_end > BUFFER_SIZE, but basically that can't happen*/

		/* We lose statbuf here, so make sure response has been sent */
		bytes_free = BUFFER_SIZE - conn->buffer_end;

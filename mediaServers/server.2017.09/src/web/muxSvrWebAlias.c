...
static int _initScriptAlias(WEB_CLIENT_CONN * conn, ALIAS_T *current1, unsigned int uri_len)
{
	...
	WEB_MSG_DEBUG( "- pathname in _initScriptAlias is: \"%s\" (\"%s\")\n", pathname, pathname + current1->real_len);
...
static ALIAS_T *_findAlias(char *uri, unsigned int urilen)
{
	...
	WEB_MSG_DEBUG(	" - looking for \"%s\" (hash=%d,len=%d)...\n", uri, hash, urilen);

	current = alias_hashtable[hash];
	while (current)
	{
		WEB_MSG_DEBUG(	"comparing \"%s\" (request) to \"%s\" (alias): \n", uri, current->fakename);
		...
		*/
		...
		{
			WEB_MSG_DEBUG(	 "Got it!\n");
			return current;
		}
		else
		{
			WEB_MSG_DEBUG(	 "Don't Got it!\n");
		}
		current = current->next;
	}
	return current;
...
int translate_uri(WEB_CLIENT_CONN * conn)
{
	...
	if (current)
	{
		...
		WEB_MSG_DEBUG("realname:'%s';\t buffer:'%s'\n", current->realname, buffer);
...
void add_alias(const char *fakename, const char *realname, ALIAS_TYPE_T type)
{
	...
	WEB_MSG_DEBUG(	" - Going to add alias: \"%s\" -=> \"%s\" (hash: %d)\n", fakename, realname, hash);
	...
	WEB_MSG_DEBUG(	"- ADDED alias: \"%s\" -=> \"%s\" hash: %d\n", fakename, realname, hash);
}

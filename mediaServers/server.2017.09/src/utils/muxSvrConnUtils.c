...
int	muxSvrConnectDataWriteFd(SERVER_CONNECT *svrConn, uint8_t *buffer, int length )
{
	...
	if (len < 0)
	{
		MUX_SVR_LOG(ERROR_TEXT_BEGIN"Write media data for '%s' error: %s"ERROR_TEXT_END, svrConn->name, strerror(errno));
#if 0
		if (errno != EAGAIN && errno != EINTR)
#else			
...
MuxStream  *muxSvrFoundStream(int isRtsp, char *feedName, MUX_SVR *muxSvr)
{
	...
	for(i=0; i< cmn_list_size(&muxSvr->svrConfig.streams); i++ ) 
	{
		...
		if(stream == NULL)
		{
			MUX_SVR_LOG( "No %d Stream is NULL" , i);
			exit(1);
		}
...
MuxFeed  *muxSvrFoundFeed(int type, char *feedName, MUX_SVR *muxSvr)
{
	...
	for(i=0; i< cmn_list_size(&muxSvr->svrConfig.feeds); i++ ) 
	{
		feed = (MuxFeed *)cmn_list_get(&muxSvr->svrConfig.feeds, i);
		if(feed == NULL)
		{
			MUX_SVR_LOG( "No %d Feed is NULL" , i);
			exit(1);
		}
...
int	muxSvrServerConnectionIterate( CONNECTION_HANDLER *connHandler, MUX_SVR *muxSvr)
{
	...
	for(i=0; i< cmn_list_size(&muxSvr->serverConns); i++ ) 
	{
		...
		if(svrConn == NULL)
		{
			MUX_SVR_LOG( "No %d Service Connection is NULL" , i);
			exit(1);
		}

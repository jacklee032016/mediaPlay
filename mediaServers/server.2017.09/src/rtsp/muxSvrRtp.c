...
int muxSvrRtpNewOutHandler(DATA_CONNECT *rtpConn, int streamIndex, struct sockaddr_in *destAddr )
{
	...
	if(streamIndex < 0)
	{
		MUX_SVR_LOG( "Stream Index is invalidate value : %d", streamIndex);
		return -EXIT_FAILURE;
	}
...
//	muxSvrMediaCodec(st, rtpConn->stream->feed->lavStreams[streamIndex]);
	muxMediaInitAvStream(st, rtpConn->stream->feed->feedStreams[streamIndex]);
...
VTRACE();
		...
		default:
			MUX_SVR_LOG( "Not support RTP low layer protocol: %d", ctx->filename, streamIndex);
			goto fail;
	}

//	MUX_SVR_DEBUG("%s:%d - - \"PLAY %s/streamid=%d %s\"\n", 	ipaddr, ntohs(dest_addr->sin_port), c->stream->filename, stream_index, c->protocol);
	MUX_SVR_DEBUG("%s: %s:%d - - [%s] \"SETUP %s/streamid=%d %d\"", rtpConn->protocol, ipaddr, ntohs(destAddr->sin_port), muxGetCtime1(buf2, sizeof(buf2)),  rtpConn->stream->filename, streamIndex, rtpConn->rtpProtocol);
//	fprintf(stderr, "%s:%d - - [%s] \"SETUP %s/streamid=%d %d\"\n", ipaddr, ntohs(destAddr->sin_port), muxGetCtime1(buf2, sizeof(buf2)),  rtpConn->stream->filename, streamIndex, rtpConn->rtpProtocol);
...
//		buf_size = IOBUFFER_INIT_SIZE;
...
#if 0
	{
		...
		MUX_SVR_LOG( "Stream pos = %lld, time=%s", stream_pos, ctime(&when));
	}
#endif
...
//	if (av_open_input_file(&avCtx, input_filename, NULL, buf_size, NULL) < 0)
	...
	{
		MUX_SVR_LOG( "%s not found", input_filename);
		return -1;
	}
...
//		muxSvrMediaCodec(st, stream->feed->lavStreams[i]);
		muxMediaInitAvStream(st, stream->feed->feedStreams[i]);
VTRACE();
	}

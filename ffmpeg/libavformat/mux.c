...
//        pkt->pts= st->cur_dts;
    ...
    if (st->cur_dts && st->cur_dts != AV_NOPTS_VALUE &&
        ...
        av_log(s, AV_LOG_ERROR,
               "Application provided invalid, non monotonically increasing dts to muxer in stream %d: %s(stream's DTS) >= %s(PKT DTS) , pkt size:%d, stream Id:%d\n",
               st->index, av_ts2str(st->cur_dts), av_ts2str(pkt->dts), pkt->size, pkt->stream_index );

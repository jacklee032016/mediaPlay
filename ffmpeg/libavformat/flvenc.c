...
static int flv_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (pkt->dts < -flv->delay) {
        av_log(s, AV_LOG_WARNING,
               "Packets are not in the proper order with respect to DTS (DTS %s < delay %s ) packet length %d on stream %d\n", av_ts2str(pkt->dts), av_ts2str(-flv->delay), pkt->size, pkt->stream_index);
        return AVERROR(EINVAL);
    }
